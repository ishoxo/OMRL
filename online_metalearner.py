...
"""
takes pretrained VAE and exploration policy
interacts with test distribution of tasks
gathers data with exploration policy for 4 episodes
exploits with exploitation policy on final episode
performance on final episode constitutes our evaluation

BUILD IN:
- collecting rollouts using two polices
- loading pretrained exploraiton policy
-change dimensions of policy inputs to include VAE belief
"""


class Online_MetaLearner:
    """
    Learner class.
    """
    ...
    def initialize_exploraiton_policy(self):
    ...
    def train(self):
        ...
        for iter_ in range(self.args.num_iters):
            ...
            if iter_ == 0:
                ...
                self.collect_rollouts_exp(num_rollouts=self.args.num_init_rollouts_pool, random_actions=True)
                print('Done!')
            # collect data from subset of train tasks

            self.env.reset_task(idx=self.task_idx)
            self.collect_rollouts_exp(num_rollouts=self.args.num_rollouts_per_iter)
            # update
    ...
    def collect_rollouts_exp(self, num_rollouts, random_actions=False):
        '''
        ...
        for rollout in range(num_rollouts):
            ...
            while not done_rollout:
                if random_actions:
                    ...
                    if self.args.policy == 'dqn':
                        action, _ = self.exp_agent.act(obs=obs)   # DQN
                    else:
                        action, _, _, _ = self.exp_agent.act(obs=obs)   # SAC
                # observe reward and next obs
                next_obs, reward, done, info = utl.env_step(self.env, action.squeeze(dim=0))
        ...
        #Now for the exploitation policy to take over:
        #do we load up pretrained policy for exploitaiton?
        obs = ptu.from_numpy(self.env.reset(self.task_idx))
        obs = obs.reshape(-1, obs.shape[-1])
        done_rollout = False
        while not done_rollout:
