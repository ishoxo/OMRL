...
class OfflineMetaLearner:
    ...
    def __init__(self, args):
        ...
        if self.args.env_name == 'GridNavi-v2_misc':
    ...
    def load_buffer(self, env):
        dataset, goals = off_utl.load_dataset_hybrid(data_dir=self.args.relabelled_data_dir,
                                                         args=self.args,
                                                         num_tasks=self.args.num_train_tasks,
                                                         IR_weight=self.args.IR_weight,
                                                         IR_type=self.args.IR_type,
    ...
    def update(self, tasks):
        ...
        for update in range(self.args.rl_updates_per_iter):
            ...
            # here is where I put my intrnisic reward function.
    ...
    def log(self, iteration, train_stats):
        ...
        if iteration % self.args.save_interval == 0:
            ...
            if not os.path.exists(save_path):
                os.mkdir(save_path)
            torch.save(self.agent.state_dict(), os.path.join(save_path, "agent{}_{}_{}.pt".format(iteration,
                                                                                                  self.args.IR_type,
                                                                                                  self.args.IR_weight
                                                                                                  )))
