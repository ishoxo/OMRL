...
class GridNavi(gym.Env):
    def __init__(self,
        ...
        self.num_tasks = len(self.possible_goals)
    ...
    def step(self, action):
        ...
        self.update_belief(self._state)
        belief_reward = self._compute_belief_reward()
        info.update({'belief_state': self._belief_state})
        # compute reward
    ...
    def goal_to_onehot_id(self, pos):
        ...
        clo = self.task_to_id(pos)  #pos is (61, 60, 2) tensor

        if clo.dim() == 1:
            clo = clo.view(-1, 1)
        clo = clo.unsqueeze(dim=-1)
        nb_digits = self.num_cells ** 2  #clo is tensor (61, 60) pos is still 61, 60, 2
        # One hot encoding buffer that you create out of the loop and just keep reusing
        y_onehot = torch.FloatTensor(pos.shape[0], pos.shape[1], nb_digits).to(ptu.device)
        # In your for loop
        y_onehot.zero_() #y onehot is 61, 25
        y_onehot.scatter_(1, clo, 1)
        return y_onehot
