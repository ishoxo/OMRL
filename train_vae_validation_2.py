...
def baseline_task_predictor_loss(dec_embedding, dec_task, vae):
    # make some predictions and compute individual losses
    #make this mean of all possible goals
    # task_pred = vae.task_decoder(dec_embedding)
    ...
    for episode_idx in range(num_episodes):
        ...
        if args.decode_task:
            ...
            if args.task_pred_type == 'task_description':
                btrl = baseline_task_predictor_loss(dec_embedding, dec_task, vae)
                btrl = btrl.sum(dim=1)
                baseline_task_recon_losses.append(btrl)
            else:
                baseline_task_recon_losses.append(trl)
