...
class VAE:
    ...
    def initialize_optimizer(self):
        ...
        self.enc_optimizer = torch.optim.Adam([*self.encoder.parameters()], lr=self.args.vae_lr)
        self.dec_optimizer = torch.optim.Adam([*decoder_params], lr=self.args.vae_lr)
    ...
    def compute_task_reconstruction_loss(self, dec_embedding, dec_task, return_predictions=False):
        ...
        if self.args.task_pred_type == 'task_id':
            env = gym.make(self.args.env_name)
            # dec_task_1 = env.task_to_id(dec_task) #ORIGINAL
            ...
            loss_task = (task_pred - dec_task).pow(2).mean(dim=1) ##dim = [61, 2]
