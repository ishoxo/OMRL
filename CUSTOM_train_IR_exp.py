...
from offline_meta_explorer import OfflineMetaLearner
...
def main(IR_type, IR_weight):
    ...
    if args.transform_data_bamdp:
        ...
        args.data_dir = "data"
        # args.data_dir = "data"
        dataset, goals = off_utl.load_dataset(data_dir=args.data_dir, args=args,
                                                     arr_type='numpy')
        bamdp_dataset = off_utl.transform_mdps_ds_to_bamdp_ds(dataset, vae, args)

        off_utl.save_data_with_exploration_rewards(save_data_path, bamdp_dataset, goals)

        # dataset, goals = off_utl.load_dataset_hybrid(data_dir=args.data_dir+"_bamdp", args=args, IR_type='KL', IR_weight=1,
        #                                              arr_type='numpy')
        # bamdp_dataset = off_utl.transform_mdps_ds_to_bamdp_ds(dataset, vae, args)
        # save relabelled data
        # off_utl.save_dataset(save_data_path, bamdp_dataset, goals)
        # off_utl.save_data_with_exploration_rewards(save_data_path, bamdp_dataset, goals)
    args.hindsight_relabelling = False
    args.IR_type = IR_type
    args.IR_weight = IR_weight
    # ipdb.set_trace()
